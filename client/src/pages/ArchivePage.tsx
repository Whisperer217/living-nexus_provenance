...
const STATUS_STYLES: Record<string, { bg: string; text: string; label: string }> = {
  ...
  Draft: {
    bg: "oklch(0.65 0.18 45 / 0.18)",
    text: "oklch(0.65 0.18 45)",
    label: "Draft",
  },
  Unlisted: {
    bg: "oklch(0.65 0.2 300 / 0.18)",
    text: "oklch(0.65 0.2 300)",
    label: "Unlisted",
  },
  Deleted: {
...
function StatusTag({ status }: { status: string }) {
  const style = STATUS_STYLES[status] ?? {
    bg: "oklch(0.5 0.03 280 / 0.18)",
    text: "oklch(0.5 0.03 280)",
    label: status,
  };
  return (
    <span
              ...
              <Link key={song.id} href={`/song/${song.id}`}>
                ...
                >
                  ...
                  <div className="flex-shrink-0">
                    <StatusTag status={song.status ?? "Published"} />
                  </div>
                </div>
              </Link>
            ))}
