...
function NotifIcon({ type }: { type: string }) {
  const map: Record<string, { icon: React.ReactNode; color: string }> = {
    witness:        { icon: <Eye size={14} />,          color: "text-[#8A6A2A] bg-[#8A6A2A]/10" },
    ...
    new_track:      { icon: <Music size={14} />,         color: "text-[#8A6A2A] bg-[#8A6A2A]/10" },
    ...
    <div
      className={`group flex items-start gap-3 p-3 rounded-xl cursor-pointer transition-all
        ${notif.isRead
          ...
          : "bg-[oklch(0.13_0.025_280)] border border-[#8A6A2A]/10 hover:border-[#8A6A2A]/20"
        }`}
      onClick={handleClick}
    >
      ...
      <div className="relative flex-shrink-0">
        ...
        {!notif.isRead && (
          <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-[#8A6A2A] border-2 border-[oklch(0.09_0.04_265)]" />
        )}
      </div>
    ...
    return (
      <div className="min-h-screen bg-[oklch(0.09_0.04_265)] flex items-center justify-center">
        <div className="text-center">
          <Bell size={40} className="mx-auto mb-4 text-[#8A6A2A]/40" />
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
            ...
            <h1 className="text-2xl font-heading text-white mb-1 flex items-center gap-2">
              ...
              {unread.length > 0 && (
                <span className="px-2 py-0.5 rounded-full text-xs font-heading
                  bg-[#8A6A2A]/10 border border-[#8A6A2A]/20 text-[#8A6A2A]">
                  {unread.length}
                </span>
              )}
            </h1>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            <Bell size={36} className="mx-auto mb-4 text-[#8A6A2A]/20" />
