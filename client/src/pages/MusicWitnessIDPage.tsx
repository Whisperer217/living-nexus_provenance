...
function HarmonicWaveform({ harmonics, playing }: { harmonics: number[]; playing: boolean }) {
  ...
  return (
    <canvas
      ...
      width={600}
      height={120}
      className="w-full rounded border border-[#8A6A2A]/20"
  ...
  const downloadCertificate = () => {
    ...
    const harmonicRows = result.harmonics.map((freq, i) => {
      ...
      return `<tr>
        ...
        <td style="color:#8A6A2A;padding:4px 12px;font-family:monospace;">${freq} Hz</td>
...
<style>
  ...
  .cert { max-width: 800px; margin: 0 auto; border: 1px solid #8A6A2A; padding: 40px; position: relative; }
  .cert::before { content: ''; position: absolute; inset: 6px; border: 1px solid rgba(201,168,76,0.2); pointer-events: none; }
  h1 { font-family: 'Orbitron', monospace; color: #8A6A2A; font-size: 22px; letter-spacing: 4px; margin: 0 0 4px; }
  h2 { font-family: 'Orbitron', monospace; color: #6ee7f7; font-size: 13px; letter-spacing: 3px; margin: 0 0 32px; }
  .divider { border: none; border-top: 1px solid #8A6A2A; margin: 24px 0; opacity: 0.4; }
  ...
  .wid { font-family: 'Orbitron', monospace; color: #8A6A2A; font-size: 20px; letter-spacing: 3px; }
  ...
  .hs { color: #8A6A2A; font-size: 13px; letter-spacing: 1px; word-break: break-all; }
...
<div class="cert">
  ...
  <div class="value" style="font-size:18px;color:#8A6A2A;">${meta.songTitle}</div>
  ...
  return (
    <div className="min-h-screen pb-32" style={{ background: "#0a0a0a", color: "#e2e8f0" }}>
      {/* Header */}
      <div className="border-b border-[#8A6A2A]/20 px-6 py-8">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-3 mb-2">
            <Shield size={28} className="text-[#8A6A2A]" />
            <h1 style={{ fontFamily: "'Orbitron', monospace", color: "#8A6A2A", fontSize: "22px", letterSpacing: "4px", fontWeight: 700 }}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-8">
        ...
        <Section title="SONG METADATA" icon={<Music size={16} />}>
                ...
                <button
                  ...
                  className={`px-3 py-1 text-xs border transition-all duration-150 ${
                    meta.moodTags.includes(mood)
                      ? "border-[#8A6A2A] text-[#8A6A2A] bg-[#8A6A2A]/10"
                      : "border-white/10 text-white/40 hover:border-[#8A6A2A]/40"
                  }`}
          ...
          <Field label="Co-Writers" className="mt-4">
            <div className="flex gap-2 mb-2">
              ...
              <button type="button" onClick={addCoWriter} className="px-3 border border-[#8A6A2A]/30 text-[#8A6A2A] hover:bg-[#8A6A2A]/10 transition-colors">
                <Plus size={14} />
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
          ...
          <Field label="Lyrics (hashed locally — never transmitted)" className="mt-4">
            <textarea
              ...
              rows={4}
              className="w-full px-3 py-2 text-sm border border-white/10 bg-white/[0.03] text-white/80 focus:outline-none focus:border-[#8A6A2A]/40 resize-none"
        ...
        <Section title="AUDIO FILE (OPTIONAL)" icon={<Upload size={16} />}>
          <div
            className="border border-dashed border-[#8A6A2A]/20 p-8 text-center cursor-pointer hover:border-[#8A6A2A]/40 transition-colors"
          ...
          >
            ...
            {audioFile ? (
              <div>
                <Music size={24} className="text-[#8A6A2A] mx-auto mb-2" />
                <p style={{ fontFamily: "'Share Tech Mono', monospace", color: "#8A6A2A", fontSize: "13px" }}>{audioFile.name}</p>
        ...
        <button
          ...
          className="w-full py-4 border border-[#8A6A2A] text-[#8A6A2A] hover:bg-[#8A6A2A]/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-200"
        ...
        {/* ── Result Panel ── */}
        {result && (
          <div className="border border-[#8A6A2A]/40 p-6 space-y-6" style={{ background: "rgba(201,168,76,0.03)" }}>
            {/* Witness ID */}
            <div>
              <Label>WITNESS ID</Label>
              <div className="flex items-center gap-3 mt-2">
                <span style={{ fontFamily: "'Orbitron', monospace", color: "#8A6A2A", fontSize: "20px", letterSpacing: "3px" }}>
                  {result.witnessId}
                </span>
            ...
            <div>
                ...
                {result.harmonics.map((freq, i) => {
                  ...
                  return (
                    <div key={i} className="border border-white/[0.06] p-3" style={{ background: "rgba(201,168,76,0.04)" }}>
                      ...
                      <div style={{ fontFamily: "'Orbitron', monospace", color: "#8A6A2A", fontSize: "14px" }}>{freq} Hz</div>
              ...
              <div className="mt-3 flex items-center gap-2">
                <span style={{ fontFamily: "'Share Tech Mono', monospace", color: "#8A6A2A", fontSize: "12px", wordBreak: "break-all" }}>
              ...
              <button
                ...
                disabled={playing}
                className="mt-4 flex items-center gap-2 px-4 py-2 border border-[#8A6A2A]/30 text-[#8A6A2A] hover:bg-[#8A6A2A]/10 disabled:opacity-50 transition-colors"
            ...
            <div className="flex flex-wrap gap-3 pt-2 border-t border-white/[0.06]">
              <button
                ...
                className="flex items-center gap-2 px-5 py-3 border border-[#8A6A2A] text-[#8A6A2A] hover:bg-[#8A6A2A]/10 transition-colors"
  ...
  return (
    <div className={className}>
      <label className="block text-xs text-white/40 mb-1" style={{ fontFamily: "'Share Tech Mono', monospace", letterSpacing: "1px" }}>
        {label}{required && <span className="text-[#8A6A2A] ml-1">*</span>}
      </label>
      {children}
    </div>
  );
...
function Input({ value, onChange, placeholder, type = "text", onKeyDown }: {
  ...
  return (
    <input
      ...
      className="w-full px-3 py-2 text-sm border border-white/10 bg-white/[0.03] text-white/80 focus:outline-none focus:border-[#8A6A2A]/40 placeholder:text-white/60"
      style={{ fontFamily: "'Share Tech Mono', monospace" }}
    />
  );
}
...
function Select({ value, onChange, options, placeholder }: {
  ...
  return (
    <select
      value={value}
      onChange={e => onChange(e.target.value)}
      className="w-full px-3 py-2 text-sm border border-white/10 bg-[#0a0a0a] text-white/80 focus:outline-none focus:border-[#8A6A2A]/40"
      style={{ fontFamily: "'Share Tech Mono', monospace" }}
    >
...
function CopyBtn({ text, id, copied, onCopy }: { text: string; id: string; copied: string | null; onCopy: (t: string, k: string) => void }) {
  return (
    ...
    >
      {copied === id ? <Check size={14} className="text-[#8A6A2A]" /> : <Copy size={14} />}
    </button>
  );
}
