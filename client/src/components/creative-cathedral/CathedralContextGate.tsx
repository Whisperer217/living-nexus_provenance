...
export function CathedralContextGate({
  ...
  return (
    <section className="space-y-4 rounded-md p-4" style={{ border: "1px solid rgba(196,154,40,0.3)", background: "rgba(0,0,0,0.72)" }}>
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-relaxed" style={{ color: "rgba(245,237,216,0.8)" }}>
      ...
      <div className="space-y-2">
        {sources.map((source) => (
          <div key={`${source.kind}:${source.label}`} className="flex min-w-0 items-start gap-2 text-sm leading-relaxed" style={{ color: "rgba(245,237,216,0.88)" }}>
      ...
      <label className="flex cursor-pointer items-start gap-3 text-sm leading-relaxed" style={{ color: "var(--ln-parchment)" }}>
        <Checkbox
          checked={consented}
