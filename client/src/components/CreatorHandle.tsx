 ...
 *
 * Desktop: hover → mini profile card floats above/below the chip
...
function CreatorMiniCard({
  ...
  return (
    <div
      className="absolute z-50 bottom-full mb-2 left-1/2 -translate-x-1/2 w-64 rounded-xl shadow-2xl overflow-hidden"
      style={{
    ...
    >
      {isLoading ? (
        <div className="p-4 flex items-center justify-center">
          <div className="w-5 h-5 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: "rgba(196,154,40,0.4)", borderTopColor: "transparent" }} />
        </div>
      ) : data ? (
        <>
          {/* Header row */}
          <div className="flex items-center gap-3 p-3 pb-2">
            ...
            <div
              ...
              style={{
                background: data.profilePhotoUrl ? "transparent" : "linear-gradient(135deg, #7C3AED, #A78BFA)",
                color: "#fff",
              }}
            >
            ...
            <div className="flex-1 min-w-0">
              ...
              {data.artistHandle && (
                <div className="text-[11px] truncate" style={{ color: "var(--ln-gold)" }}>
                  @{data.artistHandle}
                </div>
              )}
            </div>
          ...
          <div className="px-3 pb-2 flex items-center gap-3">
            {data.primaryGenre && (
              <span
                ...
                style={{ background: "rgba(196,154,40,0.1)", color: "var(--ln-gold)", border: "1px solid rgba(196,154,40,0.2)" }}
              >
                {data.primaryGenre}
              </span>
            )}
...
export function CreatorHandle({
  ...
  const profileUrl = handle ? `/creator/${handle}` : userId ? `/creator/${userId}` : undefined;
  ...
  const chip = (
    ...
    >
      ...
      {/* Pop-up card */}
      {open && userId && !noPopup && (
        <CreatorMiniCard userId={userId} onClose={() => setOpen(false)} />
      )}
    </span>
  );

  // If we have a profile URL and pop-up is disabled, wrap in a link
  ...
  // Otherwise the chip handles its own navigation via the pop-up CTA
  return chip;
}
