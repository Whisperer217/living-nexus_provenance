...
| **Broken — delivery** | Public Batch Upload still crashes | Development preview is repaired, but all public hosts return the old HTML entry bundle `index-XaSH9KYH.js`, which loads old `BatchUploadPage-D0niIpcy.js` and reproduces React #300. Requesting the expected new chunk `BatchUploadPage-CKKZfV7d.js` returns the SPA HTML fallback (`text/html`), proving that the deployed static filesystem lacks the current frontend build. The updated backend cache policy is live. [13] | Reconcile managed static-artifact deployment. Do **not** alter the batch component again; source and preview are already correct. |
| **Broken — worker activation** | Managed LN queue job cannot authenticate | Job `ln-visual-queue` was created and dispatched once, but the platform returned HTTP 403 `permission error for cron cookie` before the application handler's response shape. The task is now deliberately paused to stop repeat failures; an anonymous API probe also correctly receives HTTP 403. [7] [14] | Reconcile managed scheduler identity/deployment authentication before resuming this task. Do not claim persistent worker operation yet. |
...
The **application-code repair is complete and published**, but the site cannot be called fully repaired until the static-artifact delivery mismatch and scheduler authentication are fixed. The next operational action is to reconcile the public frontend deployment so its static filesystem contains the current entry document and hash-named assets, then correct the managed cron identity issue and resume `ln-visual-queue` only after an authenticated callback run is observed. After that, the remaining creator-owned smoke is a real single Draft/Publish and optional batch registration.
